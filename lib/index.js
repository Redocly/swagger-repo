...
const betterSwayErrors = require('./better-sway-errors');
...
exports.bundle = function(options = {}) {
  ...
  const spec = readYaml(options.mainFile || mainFile);
...
exports.validate = function(spec, options = {}, cb) {
  ...
  sway.create({ definition: spec }).then(
    function(specObj) {
      ...
      if (isErrors) {
        const message = betterSwayErrors(result.errors);
        console.log(`Validation errors: ${message}`);
      }

      if (isWarnings) {
        // FIXME: 'discrimanator' doesn't handle properly by sway so ignore warnings
        const message = betterSwayErrors(result.warnings, true);
        console.log(`Validation warnings: ${message}`);
      }
